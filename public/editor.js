(function () {
  ...
  let history = [];
  let currentTool = 'brush';   // 'brush' | 'restore' | 'erase' | 'select'
  let tolerance  = 20;
  let brushSize  = 24;
  let selection  = null;
  ...
  function init () {
    ...
    id('aiRemoveBgBtn')   ?.addEventListener('click', aiRemoveBg);
    id('smartRemoveBgBtn')?.addEventListener('click', smartRemoveBg);

    id('toolBrush')  ?.addEventListener('click', () => setTool('brush'));
    id('toolRestore')?.addEventListener('click', () => setTool('restore'));
  ...
  function openEditor (dataUrl) {
    return new Promise(resolve => {
      ...
      // Zkontroluj dostupnost AI
      fetch('/api/image/status').then(r => r.json()).then(d => {
        const btn = id('aiRemoveBgBtn');
        if (!btn) return;
        if (d.available) {
          btn.disabled = false;
          btn.title = 'AI odebere pozadí (rembg – nejlepší kvalita)';
        } else {
          btn.disabled = true;
          btn.title = 'AI nedostupné – spusťte v Dockeru s rembg';
      ...
      img.onload = () => {
        ...
        setTool('brush'); setZoom(1); resetAdjSliders();
        setBtn('undoBtn', true);
  ...
  // ── AI odebrání pozadí (server-side rembg) ────────────────────────────────
  async function aiRemoveBg () {
    const btn = id('aiRemoveBgBtn');
    const orig = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<span class="editor-spinner"></span> AI pracuje…';

    try {
      const b64 = canvas.toDataURL('image/png').split(',')[1];
      const resp = await fetch('/api/image/remove', {
        method: 'POST',
      ...
      saveHistory();
      await loadBase64IntoCanvas('data:image/png;base64,' + result);
    } catch (err) {
      alert('AI odebrání pozadí selhalo:\n' + err.message);
    } finally {
      btn.innerHTML = orig;
      btn.disabled = false;
    }
  ...
  // ── Chytrý výběr (klient – dvě fáze) ─────────────────────────────────────
  // Fáze 1: flood fill od okrajů (odstraní vnější pozadí)
  // Fáze 2: odstraní uzavřené "ostrůvky" barvy pozadí (díry uvnitř písmen O, 0, A…)
  function smartRemoveBg () {
    saveHistory();
    const im = ctx.getImageData(0, 0, canvas.width, canvas.height);
    const { data, width, height } = im;

    // Vzorky barvy pozadí z rohů (10×10 px každý roh)
    const samples = [];
    const sz = 10;
    for (let cy = 0; cy < sz; cy++) {
      for (let cx = 0; cx < sz; cx++) {
        [[cx, cy], [width-1-cx, cy], [cx, height-1-cy], [width-1-cx, height-1-cy]]
          .forEach(([px, py]) => {
    ...
    const tR = median(samples.map(s => s[0]));
    const tG = median(samples.map(s => s[1]));
    const tB = median(samples.map(s => s[2]));
    const t  = tolerance * 2.55;

    const matches = pos => {
      const i = pos * 4;
      return data[i+3] > 10 &&
        Math.abs(data[i]   - tR) <= t &&
        Math.abs(data[i+1] - tG) <= t &&
        Math.abs(data[i+2] - tB) <= t;
    };

    // ── Fáze 1: BFS od všech okrajových pixelů shodujících se s barvou pozadí
    const vis  = new Uint8Array(width * height);
    const stk  = [];
    for (let x = 0; x < width;  x++) {
      if (matches(x)) stk.push(x);
      if (matches((height-1)*width+x)) stk.push((height-1)*width+x);
    }
    for (let y = 1; y < height-1; y++) {
      if (matches(y*width)) stk.push(y*width);
      if (matches(y*width+width-1)) stk.push(y*width+width-1);
    }
    ...
    // ── Fáze 2: najdi uzavřené ostrůvky (díry v písmenech) a odstraň je
    const vis2 = new Uint8Array(width * height);
    for (let pos = 0; pos < width*height; pos++) {
      if (vis2[pos] || data[pos*4+3] === 0 || !matches(pos)) continue;
      ...
      while (q.length) {
        ...
        for (const nb of [p-1, p+1, p-width, p+width]) {
          if (nb<0 || nb>=width*height || vis2[nb]) continue;
          if (data[nb*4+3]===0 || !matches(nb)) continue;
          vis2[nb] = 1;
          q.push(nb);
        }
      }

      // Uzavřený ostrůvek = odstraň
      if (!touchesBorder) {
  ...
  // ── Načtení base64 do canvasu ──────────────────────────────────────────────
  function loadBase64IntoCanvas (dataUrl) {
    return new Promise(resolve => {
      const img = new Image();
      img.onload = () => {
        canvas.width  = img.width; canvas.height = img.height;
        syncSelCanvas(); setZoom(zoomLevel);
  ...
  function setTool (tool) {
    currentTool = tool;
    ['Brush','Restore','Erase','Select'].forEach(t =>
      id('tool'+t)?.classList.remove('active'));
    id('tool' + tool[0].toUpperCase() + tool.slice(1))?.classList.add('active');

    show('toleranceRow', tool === 'erase');
    show('brushSizeRow', tool === 'brush' || tool === 'restore');

    canvas.style.cursor = tool === 'erase'   ? 'crosshair' :
                          tool === 'select'  ? 'default'   : 'cell';
    clearSel();
  }

  // ── Magic wand (manuální flood fill na klik) ───────────────────────────────
  ...
  function floodFill (imageData, sx, sy, tol) {
    ...
    while (stk.length) {
      ...
      if (Math.abs(data[i]-tR)<=t && Math.abs(data[i+1]-tG)<=t && Math.abs(data[i+2]-tB)<=t) {
        data[i+3]=0;
        const x=p%width, y=(p/width)|0;
        if(x>0) stk.push(p-1); if(x<width-1) stk.push(p+1);
        if(y>0) stk.push(p-width); if(y<height-1) stk.push(p+width);
      }
    }
  }

  // ── Štětec (mazat / obnovit) ───────────────────────────────────────────────
  function onMouseDown (e) {
    if (currentTool==='brush' || currentTool==='restore') {
  ...
  function onMouseMove (e) {
    if (isPainting && (currentTool==='brush'||currentTool==='restore')) {
      const [x,y] = cc(e);
      if (lastPaintPt) {
  ...
  function onMouseUp (e) {
    ...
    if (isSelecting && currentTool==='select') {
      ...
      const [x,y] = cc(e);
      selection = {
        x:Math.round(Math.min(selStart.x,x)), y:Math.round(Math.min(selStart.y,y)),
    ...
    for (let py=Math.max(0,y-r|0); py<=Math.min(height-1,(y+r+1)|0); py++) {
      for (let px=Math.max(0,x-r|0); px<=Math.min(width-1,(x+r+1)|0); px++) {
        ...
        if (currentTool==='brush') {
          // Mazací štětec – průhlednost
          data[i+3] = Math.round(data[i+3]*(1-falloff));
        } else {
          // Restore štětec – obnoví původní pixel z originalImageData
          if (originalImageData) {
            const oi = (py*originalImageData.width+px)*4;
            // pouze obnovíme alpha (barva se nemění)
            const origA = originalImageData.data[oi+3];
            data[i+3] = Math.round(data[i+3] + (origA-data[i+3])*falloff);
          }
        }
      }
    }
  ...
  function onTouchStart (e) {
    ...
    } else if (currentTool==='brush'||currentTool==='restore') {
  ...
  function cropSelection () {
    ...
    ctx.putImageData(im,0,0); clearSel(); setTool('brush');
  }
  ...
  function rotate (deg) {
    ...
    canvas.widtn=nw; canvas.height=nh; syncSelCanvas();
  ...
  function flip (dir) {
    ...
    ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(off.el,0,0);
  }
  ...
  function undo () {
    ...
    ctx.putImageData(im,0,0);
    setBtn('undoBtn',!history.length); clearSel();
  }
  ...
  function reset () {
    ...
    syncSelCanvas(); ctx.putImageData(originalImageData,0,0);
  ...
  function ccXY (cx,cy) {
    ...
    return [Math.round((cx-r.left)*(canvas.width/r.width)),
